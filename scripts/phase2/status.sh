...
if [[ ! -d "$WORKTREE" ]]; then
    echo "no worktree; run phase2-bootstrap"
    exit 0
fi
...
cc=$(git -C "$WORKTREE" rev-list --count phase2/base..phase2/common 2>/dev/null || echo "?")
pc=$(find "$PROJECT_ROOT/patches/common" -maxdepth 1 -name "*.patch" 2>/dev/null | wc -l)
printf "%-30s %8s %8s\n" "common" "$cc" "$pc"
for v in $VARIANTS; do
    cc=$(git -C "$WORKTREE" rev-list --count "phase2/common..phase2/$v" 2>/dev/null || echo "?")
