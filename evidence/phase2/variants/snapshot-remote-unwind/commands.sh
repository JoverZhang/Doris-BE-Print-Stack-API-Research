#!/usr/bin/env bash
# Reproduce the snapshot-remote-unwind Tier 1 evaluation from this repo.
# Assumes:
...
#     `just phase2-bootstrap` (re-uses the existing branches;
#     snapshot-remote-unwind joins phase2-bootstrap once added to
#     scripts/phase2/_common.sh VARIANTS).
#   - .worktree/phase2 is clean.
set -euo pipefail

# 1. Switch to the variant and build/run NativeStackActionTest in the container.
just phase2-test snapshot-remote-unwind

# 2. Verify the patch series round-trips against the worktree's tree.
just phase2-verify snapshot-remote-unwind

# 3. (Optional) Regenerate patches from branch state after a commit.
# just phase2-export snapshot-remote-unwind
