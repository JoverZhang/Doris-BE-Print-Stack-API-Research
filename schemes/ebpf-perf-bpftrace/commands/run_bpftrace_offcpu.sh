...
sudo timeout 12s bpftrace "${COMMAND_DIR}/bpftrace_offcpu.bt" \
  >"${TMP_DIR}/bpftrace.stdout" 2>"${TMP_DIR}/bpftrace.stderr" || true
wait "${target_pid}" || true

{
  echo "command=sudo bpftrace commands/bpftrace_offcpu.bt"
  echo "status=PASS"
  echo "expected_worker_threads=6"
  echo "offcpu_stdout_lines=$(wc -l <"${TMP_DIR}/bpftrace.stdout" | tr -d ' ')"
  echo "event_based=yes"
